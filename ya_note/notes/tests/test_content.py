...
class TestHomePage(TestCase):
    ...
    def test_home_page(self):
        """Проверяем доступность домашней страницы и корректность шаблона."""
...
class TestNotesList(TestCase):
    ...
    def setUpTestData(cls):
        """Создаём несколько заметок для тестирования страницы списка."""
        cls.user = user()
        cls.notes = [
            Note.objects.create(title=f'Заметка {i}', text=f'Текст заметки {i}', author=cls.user)
            for i in range(5)
        ]

    def test_notes_list(self):
        """Проверяем, что страница списка заметок отображает правильное количество заметок."""
...
class TestNoteCreate(TestCase):
    ...
    def test_create_note_anonymous_user(self):
        """Проверяем, что анонимных пользователей перенаправляют на страницу входа при попытке создать заметку."""
    ...
    def test_create_note_authenticated_user(self):
        """Проверяем, что авторизованные пользователи могут получить доступ к странице создания заметки."""
        self.client.login(username='testuser', password='password')
        response = self.client.get(self.CREATE_NOTE_URL)
        self.assertEqual(response.status_code, 200)
...
class TestNoteUpdate(TestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        """Создаём заметку для редактирования."""
        cls.user = user()
        cls.note = note(cls.user)
        cls.update_url = reverse('notes:edit', args=[cls.note.slug])

    def test_update_note_authenticated_user(self, cls=None):
        """Проверяем, что авторизованные пользователи могут редактировать заметку."""
        self.client.login(username='testuser', password='password')
        response = self.client.get(cls.update_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'notes/form.html')
...
class TestNoteDelete(TestCase):
    ...
    def test_delete_note_authenticated_user(self):
        """Проверяем, что авторизованные пользователи могут удалить заметку."""
        self.client.login(username='testuser', password='password')
